...
class PartnerWithUsController extends Controller
{
  ...
  {
      $data = PartnerWithUs::all();
      return view('admin.partner_with_us.index', compact("data"));
  }
...
{
  $data = PartnerWithUs::findOrFail($id);
  return view('admin.partner_with_us.edit', compact('data'));
}
...
public function update(Request $request, $id)
{
  ...
  $about = PartnerWithUs::findOrFail($id);


  if ($request->hasFile('image')) {
  
      if ($about->image) {
          $oldFilePath = public_path('uploads/partner_with_us/' . $about->image);
      ...
      $data['image'] = $image;
      $request->image->move(public_path('uploads/partner_with_us/'), $image);
  }
