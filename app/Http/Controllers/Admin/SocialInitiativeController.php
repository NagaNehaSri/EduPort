...
class SocialInitiativeController extends Controller
{
    ...
    {
        $data = SocialInitiative::all();
        return view('admin.social_intiatives.index', compact("data"));
    }
...
{
    $data = SocialInitiative::findOrFail($id);
    return view('admin.social_intiatives.edit', compact('data'));
}
...
public function update(Request $request, $id)
{
    ...
    $data = $this->validate($request, [
        ...
        'image_2' => 'nullable|mimes:jpg,jpeg,png,gif,webp,svg',
    ]);

    
    $about = SocialInitiative::findOrFail($id);

 
    if ($request->hasFile('image')) {
    
        if ($about->image) {
            $oldFilePath = public_path('uploads/social_initiatives/' . $about->image);
        ...
        $data['image'] = $image;
        $request->image->move(public_path('uploads/social_initiatives/'), $image);
    }
    

    if ($request->hasFile('image_2')) {
        if ($about->image_2) {
            $oldFilePath = public_path('uploads/social_intiatives/' . $about->image_2);
            if (file_exists($oldFilePath)) {
                unlink($oldFilePath);
            }
        }
        ...
        $request->image_2->move(public_path('uploads/social_intiatives/'), $image_2);
    }
    ...
    public function destroy($id)
    {
        ...
        if ($about->image) {
            $oldFilePath = public_path('uploads/social_intiatives/' . $about->image);
            if (file_exists($oldFilePath)) {
                unlink($oldFilePath);
            }
        }
