            ...
            <div class="col-12">
                <div class="card">
                    <div class="card-body">
                        ...
                        <div class="row">
                            <div class="col-md-6">
                                <h4 class="page-title">Blog Slider Images</h4>
                            </div>
                            <div class="col-md-6">
                                <a href="{{ route('blog_slider_image.create', ['blog_id' => $blog_id]) }}">
                                    <button class="btn btn-success btn-sm float-right ml-2">
                                        <i class="fas fa-plus"></i> Add Blog Slider Images
                                    </button>
                                </a>
                            </div>
                        </div>
                            ...
                            <table id="example" class="table table-striped table-bordered dt-responsive nowrap" style="border-collapse: collapse; border-spacing: 0; width: 100%;">
                                ...
                                <tbody>
                                    ...
                                    <tr>
                                        <td>{{ $key + 1 }}</td>
                                        <td>
                                            @if (!empty($data->blog_slider_image))
                                            <a href="{{ asset('uploads/blog_slider_image/' . $data->blog_slider_image) }}" target="_blank">
                                                <img class="avatar-lg" src="{{ asset('uploads/blog_slider_image/' . $data->blog_slider_image) }}" alt="Project Image">
                                            </a>
                                            @else
                                        ...
                                        <td>
                                            <a href="{{ route('blog_slider_image.edit', $data->id) }}" class="btn btn-primary waves-effect waves-light btn-xs">
                                                <i class="fas fa-edit"></i>
                                            </a>
                                            <button data-value="{{ $data->id }}" class="btn btn-danger waves-effect waves-light btn-xs cdelete"><i class="fas fa-trash"></i></button>
                                            <form id="deleteform{{ $data->id }}" method="post" action="{{ route('blog_slider_image.destroy', [$data->id]) }}">
                                                @csrf
                                                {{ method_field('DELETE') }}
                                            </form>
                                        </td>
