            ...
            <div class="col-12">
                <div class="card">
                    ...
                    <div class="card-body">
                        ...
                        <div class="row">
                            <div class="col-md-12">
                                <a href="{{route('flat.index')}}" class="btn btn-dark float-right">← Go Back Flat</a>
                                <h4 class="page-title">Slides</h4>

                                <form action="{{ route('flat.slide_store') }}" method="post"
                                ...
                                <tbody>
                                    @foreach ($data as $key => $data)
                                        <tr>
                                            ...
                                            <td>
                                                @if ($data->image != '')
                                                    <a href="{{ asset('uploads') }}/flats/{{ $data->image }}"
                                                        target="_blank">
                                                        <img class="avatar-lg"
                                                            src="{{ asset('uploads') }}/flats/{{ $data->image }}"
                                                            alt="{{ $data->heading }}">
                                                    </a>
                                                @else
                                            ...
                                            <td>
                                                <a href="{{ route('flat.slide_edit', [$data->id]) }}"
                                                ...
                                                <form id="deleteform{{ $data->id }}" method="post"
                                                    action="{{ route('flat.slide_destroy', [$data->id]) }}">
                                                    @csrf
                                                    {{ method_field('DELETE') }}
                                                </form>
                                            </td>
