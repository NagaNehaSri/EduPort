                    ...
                    <div class="page-title-box">
                        <div class="page-title-right">
                            <ol class="breadcrumb m-0">
                                ...
                                <li class="breadcrumb-item"><a href="{{ route('home_about.index') }}">Home About</a></li>
                                <li class="breadcrumb-item active">Home About Images</li>
                            </ol>
                        </div>
                        @section('page_title')
                        Home about Images
                ...
                <div class="col-12">
                    <div class="card">
                        <div class="card-body">
                            ...
                            <div class="row">
                                <div class="col-md-6">
                                    <h4 class="page-title">Home About Images</h4>
                                </div>
                                <div class="col-md-6">
                                    <a href="{{ route('home_about_image_slider.create', ['home_about_id' => $home_about_id]) }}">
                                        <button class="btn btn-success btn-sm float-right ml-2">
                                            <i class="fas fa-plus"></i> Add About Images
                                        </button>
                                    </a>
                                </div>
                            </div>
                                    ...
                                    <tbody>
                                        @foreach ($data as $key => $data)
                                            <tr>
                                                <td>{{ $key + 1 }}</td>
                                                <td>
                                                    @if (!empty($data->home_about_image))
                                                        <a href="{{ asset('uploads/home_about_image/' . $data->home_about_image) }}" target="_blank">
                                                            <img class="avatar-lg" src="{{ asset('uploads/home_about_image/' . $data->home_about_image) }}" alt="Project Image">
                                                        </a>
                                                    @else
                                                        <span class="badge badge-warning">No Image</span>
                                                    @endif
                                                </td>
                                                <td>{{ \Carbon\Carbon::parse($data->updated_at)->format('d M Y h:i A') }}</td>
                                                <td>
                                                    <a href="{{ route('home_about_image_slider.edit', $data->id) }}" class="btn btn-primary waves-effect waves-light btn-xs">
                                                        <i class="fas fa-edit"></i>
                                                    </a>
                                                    <button data-value="{{ $data->id }}" class="btn btn-danger waves-effect waves-light btn-xs cdelete"><i class="fas fa-trash"></i></button>
                                                    <form id="deleteform{{ $data->id }}" method="post" action="{{ route('home_about_image_slider.destroy', [$data->id]) }}">
                                                        @csrf
                                                        {{ method_field('DELETE') }}
                                                    </form>
                                                </td>
